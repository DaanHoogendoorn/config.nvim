...
local function get_lazyblocks()
  ...
  local cmd_get_blocks = string.format('cd %s && wp post list --post_type=lazyblocks --fields=post_name,ID --format=json 2>/dev/null', vim.fn.shellescape(cwd))
...
-- Return the function so it can be used elsewhere if needed
return lazyblock_info
